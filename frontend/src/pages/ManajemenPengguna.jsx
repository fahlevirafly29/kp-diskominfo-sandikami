...
import { Users, UserPlus, List, CheckCircle2, AlertCircle } from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Kelola hak akses dan akun sistem Sandikami.</p>
                </div>
            </div>

            {/* NOTIFIKASI PESAN */}
            {message && (
                <div className={`px-4 py-3 rounded-md border flex items-center gap-3 ${message.includes('berhasil') ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                    {message.includes('berhasil') ? <CheckCircle2 size={18} /> : <AlertCircle size={18} />}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-fit">
                    <div className="flex items-center gap-2 mb-5 pb-3 border-b border-slate-100">
                        <UserPlus size={18} className="text-[#0B4A99]" />
                        <h2 className="text-base font-bold text-slate-800">Tambah Akun Baru</h2>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Role / Hak Akses</label>
                            <select 
                ...
                <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-6 pb-4 border-b border-slate-100 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-base font-bold text-slate-800">Daftar Pengguna Sistem</h2>
                        </div>
                    ...
                    <div className="overflow-x-auto p-4 pt-0">
                        <table className="w-full text-left border-collapse mt-2 rounded-lg overflow-hidden border border-slate-200">
                            <thead className="bg-slate-100 text-slate-700 font-semibold border-b border-slate-200">
                                <tr>
                                    <th className="px-4 py-3 text-xs">Nama Lengkap</th>
                                    <th className="px-4 py-3 text-xs">Username</th>
                                    <th className="px-4 py-3 text-xs text-center">Hak Akses</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {users.map((user) => (
                                    <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-4 py-3 text-slate-800 text-sm font-medium">{user.name}</td>
                                        <td className="px-4 py-3 text-slate-500 text-sm">{user.username}</td>
                                        <td className="px-4 py-3 text-sm text-center">
                                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wide uppercase border
                                                ${user.role === 'admin' ? 'bg-red-50 border-red-200 text-red-600' : 
                                                  user.role === 'kadis' ? 'bg-amber-50 border-amber-200 text-amber-600' : 
